package com.special.ResideMenuDemo.lib;
...
import com.special.ResideMenuDemo.lib.internal.LoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			...
			/*houmingtao add start*/
			if(lastUpateTime!=null && lastUpateTime.trim().length()>0)
			{
				pullLabel+=context.getString(R.string.last_update_time,lastUpateTime);
			}
			/*houmingtao add end*/
...
//				newHeight = Math.round((initialMotionY - lastMotionY) / FRICTION);
	...
	public static interface OnRefreshListener {
		...
		public void onRefresh(int freshMode);
	...
	public void markLastUpateTime()
	{
		lastUpateTime = DateUtil.getStringdateYMDHMS(new Date());
	}
	
	public void markLastUpateTime(Date date)
	{
		lastUpateTime = DateUtil.getStringdateYMDHMS(date);
	}
	
	public void setLastUpateTime(String v_lastUpateTime)
	{
		lastUpateTime = v_lastUpateTime;
	}
	/*houmingtao add end*/
}
